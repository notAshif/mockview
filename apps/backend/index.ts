...
wss.on('connection', async (ws: WebSocket, request?: any, interviewId?: string) => {
    const reqUrl = new URL(request.url || "", "http://localhost");
    const personaId = reqUrl.searchParams.get("persona") || "elena";

    console.log(`[Server] WebSocket connection opened for interview ${interviewId} (Persona: ${personaId})`);
    
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
        console.error("[Server] GEMINI_API_KEY is not defined in env");
        ws.send(JSON.stringify({ type: "error", message: "GEMINI_API_KEY is not set on backend" }));
        ws.close();
        return;
    }
    ...
    const systemPrompts: Record<string, string> = {
        elena: "You are Elena Rostova, a Principal System Design Expert conducting a technical mock interview for a Computer Science student. " +
        ...
        marcus: "You are Marcus Vance, an HR Behavioral Lead conducting a mock behavioral interview for a software engineering position. " +
        ...
        sarah: "You are Sarah Chen, a Frontend Tech Lead conducting a mock frontend architecture interview. " +
            ...
            if (response.inputTranscription) {
                ...
                if (response.inputTranscription.turnComplete) {
                    ...
                    if (finalizedText) {
                        ...
                        ws.send(JSON.stringify({ type: "user_transcript", text: finalizedText }));
    ...
    geminiWs.on('error', (err) => {
        ...
        ws.send(JSON.stringify({ type: "error", message: "Gemini session error occurred" }));
    });

    geminiWs.on('close', (code, reason) => {
        console.log(`[Server] Gemini connection closed: ${code} - ${reason.toString()}`);
        ...
        if (isBinary) {
            ...
            try {
                ...
                if (payload.type === "text" && payload.text) {
                    ...
                    }).catch(err => console.error("Failed to update interview status:", err));
    ...
    ws.on('close', () => {
        console.log(`[Server] Client WebSocket closed for interview ${interviewId}`);
